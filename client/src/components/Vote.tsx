...
const Vote = ({
  ...
  if (loading) {
    return (
      <div className="flex items-center bg-darkCharcoal p-2 rounded-full cursor-not-allowed">
        ...
        >
          <BiSolidUpvote className={`${isMobile ? "text-xl" : "text-xl"}`} />
        </button>
        ...
        <button
          className="flex items-center text-steelGray cursor-not-allowed"
        >
          <BiSolidDownvote className={`${isMobile ? "text-xl" : "text-xl"}`} />
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center border border-steelGray px-2 py-1 rounded-full hover:cursor-pointer">
      <button
        disabled={loading}
      ...
      >
        {pressed ? (
          <BiSolidUpvote className={`${ isMobile ? ("text-xl") : ("text-xl")} text-cornflowerBlue`} />
        ) : (
          <BiUpvote className={`${ isMobile ? ("text-xl") : ("text-xl")}`} />
        )}
      </button>
      {/* Display votes */}
      <span className={`ml-2 ${ isMobile ? ("text-md") : ("text-md")} text-crispWhite`}>{votes}</span>
      <span className={`mx-3 ${ isMobile ? ("text-md") : ("text-md")} text-steelGray`}>|</span> {/* Separator */}
      <button
        disabled={loading}
        className={`flex items-center text-crispWhite hover:text-cornflowerBlue ${
          loading && "opacity-50 cursor-not-allowed"
      ...
      >
        {pressedDownvote ? (
          <BiSolidDownvote className={`${ isMobile ? ("text-xl") : ("text-xl")} text-cornflowerBlue`} />
        ) : (
          <BiDownvote className={`${ isMobile ? ("text-xl") : ("text-xl")}`} />
        )}
      </button>
    </div>
  );
